...
const getFiles = async (req, res) => {
  try {
    ...
    const { ref_id, financial_year, categary } = req.params;   // FIXED

    const request = pool.request();
    request.input("ref_id", sql.VarChar(12), ref_id);
    request.input("financial_year", sql.VarChar(9), financial_year);
    request.input("categary_cd", sql.VarChar(2), categary);     // FIXED
...
const uploadFile = async (req, res) => {
  try {
    ...
    request.input("categary_cd", sql.VarChar(2), body.category);   // FIXED
