...
const ClientFileUpload = () => {
  ...
  // ⬅️ NEW REF for hidden edit file input
  const editFileInputRef = useRef(null); 
  // ⬅️ NEW STATE to track which file's SNO we are about to replace
  ...
  // Removed unused editData and editFile states as they are handled in the new flow
...
const handleEditFileChange = async (e) => {
  ...
  try {
    // 🔹 Make sure sno type matches (important)
    ...
    // formData.append("ref_id", ref_id);
    // formData.append("financial_year", financial_year);
    // formData.append("sno", fileToReplaceSno);
    // formData.append("user_id", user_id);
    // formData.append("user_name", user_name);
    // formData.append("file", newFile); // MUST be last (multer best practice)


    const formData = new FormData();

// 🔥 REQUIRED for multer filename()
formData.append("ref_id", ref_id);
formData.append("categary_cd", originalFile.categary_cd);
formData.append("nextCount", originalFile.sno); // use same sno for replace

// 🔥 Required for controller + DB
formData.append("financial_year", financial_year);
formData.append("sno", fileToReplaceSno);
formData.append("user_id", user_id);
formData.append("user_name", user_name);

// File MUST be last (good practice)
formData.append("file", newFile);


    const res = await axios.put(
      "http://localhost:3080/api/files", // <-- matches your controller
      formData,
      {
        headers: {
  ...
  return (
    ...
    >
      <div className="container">
                  ...
                  return (
                    <tr key={file.sno}>
                      ...
                      <td className="d-flex justify-content-around">
                        ...
                        {/* ⬅️ CORRECTED EDIT BUTTON */}
                        <button 
                            className="btn btn-warning"
                            // Set the SNO and trigger the hidden file input click
                            onClick={() => startEditProcess(file.sno)}
                        >
                            <FaPencilAlt />
                        </button>
                      </td>
        ...
        {/* ⬅️ HIDDEN INPUT ELEMENT FOR EDITING */}
        <input
            type="file"
            ref={editFileInputRef}
            onChange={handleEditFileChange} // The function that handles the PUT request
            style={{ display: "none" }}
        />
      ...
      <div className="text-center mt-3">
        <button 
            className="btn px-4 text-white me-4"
            style={{
